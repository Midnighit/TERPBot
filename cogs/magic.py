...
from checks import has_role_greater_or_equal
from config import PREFIX, SUPPORT_ROLE
from exiles_api import session, MagicChars, Characters, Users
...
class Mag(commands.Cog, name="Magic commands."):
    ...
    async def get_char_by_user(user, single_only=True):
        ...
        if len(chars) > 0 and not single_only:
            ...
            return (
                f"User **{user.disc_user}** has more than one registered magic character. "
                f"Please specify character name."
            )
        return chars[0]
    ...
    async def add(self, ctx, *, Character):
        ...
        if mchar and char.name != mchar.name:
            ...
            if not mchar.active:
                ...
                logger.info(f"{anc} Registered {char.name} as magic user.")
            else:
                await ctx.send(
                    f"Character is already registered under name **{mchar.name}**. "
                    f"Name has been updated to **{char.name}**."
                )
                logger.info(
                    f"{anc} Character is already registered under name {mchar.name}. "
                    f"Name has been updated to {char.name}."
                )
            session.commit()
            return
            ...
            if not mchar.active:
                ...
                logger.info(f"{anc} Character is already registered. No changes were made.")
            return
        ...
        logger.info(f"{anc} Registered {char.name} as a magic user.")
    ...
    async def use(self, ctx, Mana: int, *Character):
        ...
        if not mchar or not mchar.active:
            ...
            logger.info(f"{anc} {char.name} is not registered. No changes were made.")
            return
        elif mchar.mana < Mana:
            await ctx.send(f"**{char.name}** does not have enough mana. Mana points left this week: **{mchar.mana}**.")
            logger.info(f"{anc} {char.name} does not have enough mana. Mana points left this week: {mchar.mana}.")
            return
        mchar.mana -= Mana
        mchar.total_uses += 1
        ...
        logger.info(f"{anc} {char.name} used {Mana} {mp}. Mana points left this week: {mchar.mana}.")
    ...
    async def give(self, ctx, Mana: int, *Character):
        ...
        if character == "":
            ...
            logger.info(f"{anc} Character is a required argument that is missing.")
            return
        # determine the char by its name or id or return an error message if none were found
        result = await Mag.get_char(character)
        if isinstance(result, str):
        ...
        logger.info(f"{anc} {char.name} was given {Mana} {mp}. Mana points left this week: {mchar.mana}.")
    ...
    async def undo(self, ctx, *Character):
        ...
        if not mchar or not mchar.active:
            ...
            await ctx.send(
                f"**{char.name}** has not used any mana so far or there is no record of it. " f"No changes were made."
            )
            logger.info(
                f"{anc} {char.name} has not used any mana so far or there is no record of it. No changes were made."
            )
            return
        ...
        await ctx.send(
            f"**{char.name}** was given back **{given}** {mp}. Mana points left this week: **{mchar.mana}**."
        )
        logger.info(f"{anc} {char.name} was given {given} {given}. Mana points left this week: {mchar.mana}.")

    @mag.command(
        name="stats", help="Shows statistics for all or a single character or resets them for a single character"
    )
    @has_role_greater_or_equal(SUPPORT_ROLE)
    async def stats(self, ctx, *args):
        ...
        if len(args) > 0 and args[0] == "reset":
            ...
            logger.info(f"{anc} {char.name} has been reset to 0 mana and 0 uses.")
        # display the stats for the given character
            ...
            await ctx.send(
                f"**{char.name}** has **{mchar.mana}** {mp} left this week, used mana **{mchar.total_uses}** {tm} "
                f"and spent **{mchar.total_spent}** {sp} that way since registration."
            )
            logger.info(
                f"{anc} {char.name} has {mchar.mana} {mp} left this week, used mana {mchar.total_uses} {tm} "
                f"and spent {mchar.total_spent} {sp} that way since registration."
            )
        # display the stats for all registered users
            ...
            if len(mchars) == 0:
                ...
                logger.info(f"{anc} No magic chars registered.")
                return
    ...
    async def mana(self, ctx):
        ...
        if len(output) > 0:
            ...
            await ctx.send("No registered characters found.")
