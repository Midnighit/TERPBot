...
class BBK(commands.Cog, name="Boatbucks commands."):
        ...
        if ctx.author.id in self.permitted:
            ...
            if ctx.invoked_subcommand is None:
                ...
                if user is None or user.bucks == 0:
                    await ctx.send(
                        f"You don't have any {self.bbk}. "
                        f"Maybe you can earn some if you ask <@{self.master_id}> nicely!"
                    )
                elif user.bucks < 0:
                    await ctx.send(
                        f"You are {abs(user.bucks)} {self.bbk} in boatdebt. "
                        f"Maybe you can earn some if you ask <@{self.master_id}> nicely!"
                    )
                else:
        ...
        if ctx.author.id in self.permitted:
            ...
            if len(blocks) == 0:
                await ctx.author.send(
                    (
                        "It seems like nobody is using the boatback right now. Shame really. "
            ...
            await ctx.send(
                (
                    "Nope, no can do. The boatbank very much values the privacy of its customers...\n"
                    "most of the times...\nif it suits us..."
                )
            )
        logger.info(f"Author: {ctx.author} / Command: {ctx.message.content}.")
    ...
    async def give(self, ctx, bucks: int, member: Member):
        ...
        # sender and recipient of the boatbucks are the same person
            ...
            replies = [
                (
                    "Trying to send boatbucks to yourself, huh? "
        ...
        # sender can create and take away boatbucks at will
            ...
            replies = [
                (
                    f"Sure boss. Created **{bucks}** {self.bbk} out of thin air for {member.mention}. "
            ...
            if not sender or sender.bucks < 0:
                ...
                if not sender or sender.bucks == 0:
                    replies = [
                        (
                            f"You don't have a single penny, much less a whole boatbuck right now. "
                ...
                # sender doesn't have as many bucks as they want to send
                    ...
                    replies = [
                        (
                            f"Nice try but you only have **{sender.bucks}** {self.bbk}. "
                ...
                if member.id in self.permitted:
                    replies = [
                        (f"Destroying **{bucks}** {self.bbk} for you. " f"{member.mention} doesn't need them anyway."),
                        (
                ...
                # default case sender and recipient are normal and sender has enough bucks
                    ...
                    replies = [
                        (
                            f"Alright, transferring **{bucks}** {self.bbk} from your account to {member.mention}. "
    ...
    async def give_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.MissingRequiredArgument):
            ...
            await ctx.send("You need to pay in boatbucks not in members dummy!")
        else:
            pe(error)
            logger.error(f"Author: {ctx.author} / Command: {ctx.message.content}. {error}")
    ...
    async def take(self, ctx, bucks: int, member: Member):
        ...
        if ctx.author.id not in self.permitted:
            replies = [
                (
                    f"No can do. Only <@{self.master_id}> and her henchmen from the boatbank are allowed to "
        ...
        # recipient can create and take away boatbucks at will
            ...
            replies = [
                (
                    "Hey, you can't take away boatbucks from a fellow employee at the boatbank. "
            ...
            if recipient.bucks == bucks:
                ...
                replies = [
                    (
                        f"First they took {member.mention}'s family and boatbucks, then they took their "
                ...
                replies = [
                    (
                        f"Huh, I guess you really don't like {member.mention}. Took {bucks} {self.bbk} from them "
                ...
                replies = [
                    (
                        f"With pleasure boss. {member.mention} is now **{bucks}** "
    ...
    async def take_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.MissingRequiredArgument):
            if error.param.name == "member":
                ...
                await ctx.send(
                    "While I'd love to take away other peoples hard earned boatbucks, " "you need to tell me how many!"
                )
        elif isinstance(error, commands.errors.BadArgument):
            await ctx.send(f"Sorry but who tf is **{error.argument}** supposed to be?")
        elif isinstance(error, ValueError):
            await ctx.send("You need to take boatbucks not members dummy!")
        else:
            pe(error)
            logger.error(f"Author: {ctx.author} / Command: {ctx.message.content}. {error}")
    ...
    async def tax(self, ctx):
        ...
        if ctx.author.id not in self.permitted:
            replies = [
                (
                    f"Trying to collect taxes on behalf of the boatbank? While that's very commendable only "
        ...
        if len(recipients) == 0:
            if session.query(func.count(Boatbucks.id)).scalar() == 0:
                ...
                replies = [
                    (f"It looks like nobody on the boatbanks has any {self.bbk} left to pay taxes with. " "What gives?")
                ]
        else:
            ...
            if member:
                ...
                replies = [
                    (f"The taxboat cometh and has taken one {self.bbk} from {member.mention}! {leftover}"),
                    (
                ...
                replies = [
                    (
                        "Huh, looks like nobody who could afford a boattax is on this discord. "
